...
//Rare exceptions to indicate a bug in the _regionslocator code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_regionslocator = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_regionslocator]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.regionslocator.module"
