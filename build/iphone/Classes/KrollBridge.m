...
NSString * RegionsLocator$ModuleRequireFormat = @"(function(exports){"
...
@implementation RegionsLocatorObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_regionslocator);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create RegionsLocator global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "RegionsLocator" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_regionslocator = [[RegionsLocatorObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_regionslocator];
	
	NSString *_regionslocatorNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _regionslocatorNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_regionslocator valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:RegionsLocator$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.regionslocator.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.regionslocator.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
